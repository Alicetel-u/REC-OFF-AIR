...
- **デバッグ用フラグ（`const DEBUG_* := true` や `const SAFE_* := true` 等）を依頼と無関係な箇所に勝手に追加しない**
- テスト効率化のためにコードを変えた場合は、コミットメッセージおよびユーザーに必ず報告する
- デバッグフラグを追加した場合、そのフラグが影響する**すべての分岐**で元の動作（BGM・SE・演出等）が維持されているか確認してからコミットする
- `Opening.gd` の `SAFE_BOOT_TITLE` を変更するときは、`_show_safe_title()` の BGM 再生も必ず合わせて確認する
