...
func _ready():
	...
	# 他の _ready() が光源設定を上書きした後に確実に適用する
	call_deferred("_setup_light")


# 遅延実行でライト設定を強制適用
func _setup_light() -> void:
	light.light_energy = 3.5   # 向けた先は見えるが氾濫しない強さ
	light.spot_range   = 22.0  # 22m先は闇に沈む
	light.spot_angle   = 35.0  # 狭いビームで周辺が見えない恐怖
	light.visible      = (current_mode == CAM_MODES.NIGHT_VISION_ON)
	# Godot 4 Forward Plus は Reversed-Z バッファを使うため
	# POSITION = vec4(xy, 1.0, 1.0) がニアプレーンに配置され
	# 3D シーン全体を遮蔽して真っ暗になるバグを防ぐ
	night_vision_mesh.visible = false
